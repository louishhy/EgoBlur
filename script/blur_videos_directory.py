...
def parse_args():
    parser = argparse.ArgumentParser(description="Apply blurring to videos based on JSONL detection data")
    parser.add_argument(
        "--input_directory",
        required=True,
        type=str,
...
def find_target_videos_with_jsonl(root_directory: str, logger: logging.Logger) -> List[Tuple[str, str, str]]:
    """
    ...
    for target_file in target_files:
        ...
        for video_path in root_path.rglob(target_file):
            # Check for corresponding JSONL file
            video_dir = video_path.parent
            jsonl_name = target_file.replace("_fused.mp4", "_egoblur_detection.jsonl")
...
def process_directory(
    ...
    video_files = find_target_videos_with_jsonl(args.input_directory, logger)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info("Starting EgoBlur video blurring")
